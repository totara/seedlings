...
require_capability('enrol/totara_facetoface:config', $context);

$PAGE->set_url('/enrol/totara_facetoface/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'totara_facetoface', 'id'=>$instanceid), '*', MUST_EXIST);
} else {
...
$mform = new enrol_totara_facetoface_edit_form(null, array($instance, $plugin, $context));
...
$PAGE->set_title(get_string('pluginname', 'enrol_totara_facetoface'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_totara_facetoface'));
$mform->display();
echo $OUTPUT->footer();
