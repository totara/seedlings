...
class core_outputcomponents_testcase extends advanced_testcase {
    ...
    public function test_get_url() {
        ...
        $this->assertTrue(file_exists("$CFG->dirroot/theme/kiwifruit/config.php"));
        set_config('theme', 'kiwifruit');
        $page = new moodle_page();
        $page->set_url('/user/profile.php');
        $page->set_context(context_system::instance());
        $renderer = $page->get_renderer('core');
        ...
        $up2 = new user_picture($user2);
        $this->assertSame('http://www.gravatar.com/avatar/ab53a2911ddf9b4817ac01ddcd3d975f?s=35&d=http%3A%2F%2Fwww.example.com%2Fmoodle%2Ftheme%2Fkiwifruit%2Fpix_core%2Fu%2Ff2.png', $up2->get_url($page, $renderer)->out(false));

        // Https version.
        $CFG->httpswwwroot = str_replace('http:', 'https:', $CFG->wwwroot);

        $up2 = new user_picture($user2);
        $this->assertSame('https://secure.gravatar.com/avatar/ab53a2911ddf9b4817ac01ddcd3d975f?s=35&d=https%3A%2F%2Fwww.example.com%2Fmoodle%2Ftheme%2Fkiwifruit%2Fpix_core%2Fu%2Ff2.png', $up2->get_url($page, $renderer)->out(false));
        // end of gravatar tests

        // Test themed images.
        set_config('enablegravatar', 0);
        $this->assertFileExists("$CFG->dirroot/theme/customtotara/config.php"); // use any other theme
        ...
        $up1 = new user_picture($user1);
        $this->assertSame($CFG->wwwroot.'/pluginfile.php/'.$context1->id.'/user/icon/customtotara/f2?rev=11', $up1->get_url($page, $renderer)->out(false));

        $up2 = new user_picture($user2);
        $this->assertSame($CFG->wwwroot.'/theme/image.php/customtotara/core/1/u/f2', $up2->get_url($page, $renderer)->out(false));
